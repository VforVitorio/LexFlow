    ...
    <div className={cn('rounded-lg border border-border bg-surface px-6 py-5', className)}>
      ...
      <div className="relative pb-7 pt-5">
        ...
        {versions.map((v, i) => {
          ...
          // Anchor the first label to the left edge and the last to the right edge
          // so they never bleed outside the container. All others stay centred.
          const translateX = isFirst ? 'translateX(0%)' : isLast ? 'translateX(-100%)' : 'translateX(-50%)';
          return (
            <button
              key={v.tag}
              onClick={() => onSelect?.(v.tag)}
              title={v.tag}
              className="absolute top-5 text-center hover:cursor-pointer"
              style={{ left: `${pct}%`, transform: translateX }}
            >
              <div className={cn(
              ...
              <div className="mt-2 font-mono text-[10.5px] font-semibold">{v.date.slice(0, 7)}</div>
            </button>
          );
        })}
      </div>
