...
- **2026-07-05** — En el mapa interactivo de Comunidades (#846/PR #853) verifiqué `typecheck` + `eslint` + `vite build` en local y pusheé, pero **NO corrí `vitest`** → CI (`frontend-build`) rojo: cambiar la vista por defecto a mapa rompió los 4 tests de `CommunitiesPage.test.tsx` que asumían el grid en el mount, y añadir Ceuta/Melilla a `COMMUNITIES` descuadró el conteo de tarjetas. Causa: `vite build` NO ejecuta los tests; typecheck/eslint/build pueden pasar con tests rotos, y un cambio de UI (vista por defecto, nueva entrada en una lista compartida) puede romper tests existentes que ni sabía que existían. Fix: antes de pushear CUALQUIER cambio de frontend con superficie de comportamiento, correr la suite COMPLETA `npx vitest run` (no solo `build`), igual que en backend se corre `pytest` entero (lección 2026-06-09). Regla: al tocar un componente con test co-localizado o una constante compartida (`COMMUNITIES`, tipos, stores), `grep` sus tests y correr `vitest run` completo. Cómo arreglarlo si ya pasó: actualizar los tests al nuevo comportamiento (scopear asserts al contenedor correcto cuando aparecen botones nuevos como un toggle), `vitest run` verde, commit `test(...)`, push a la MISMA PR.

- **2026-06-13** — `gh pr merge --auto` sobre una PR de CI rápida **mergea en el PRIMER commit que pone los checks en verde**: habilité auto-merge tras el commit 1 (fix `@v2` de OSV-Scanner) y luego pushé los commits del torch-ignore → la PR (#638) se mergeó solo con el commit 1 y dejó `main` ROJO en OSV-Scanner (el ignore quedó varado en la rama, hubo que rehacerlo en #642). Causa: con CI que tarda segundos, auto-merge dispara antes de que lleguen los commits siguientes; los pushes posteriores van a una PR ya cerrada. Fix: meter TODO el cambio en UN commit antes de `--auto`, o no usar `--auto` y hacer `gh pr merge --merge` cuando TODOS los commits estén pusheados y en verde. Regla: nunca pushear commits de seguimiento después de armar `--auto` en una PR de checks rápidos.
- **2026-06-13** — El reusable de OSV-Scanner `osv-scanner-reusable.yml@v1` dejó de resolver (OSV-Scanner saltó a v2) → cada run moría al arrancar con **0 jobs** y el escaneo nunca corría (cobertura de vulns silenciosamente apagada; rojo en `main` y en la PR de release-please). Una llamada a un reusable que no resuelve falla el workflow entero antes de cualquier job (señal: run failure con 0 jobs). Fix: pinear a un tag real (`@v2.3.8`) y dejar que Dependabot (ecosistema github-actions) lo suba; añadir el propio workflow + `osv-scanner.toml` al filtro `paths` del `pull_request` para que un bump roto se vea EN la PR y no solo post-merge. Vulns sin parche upstream (p.ej. torch del extra opt-in `[semantic]`) → `osv-scanner.toml` con `[[IgnoredVulns]]` documentado y con `ignoreUntil` (fecha de revisión), no desactivar el scanner.
- **2026-06-09** — Añadir campos REQUERIDOS (`rerank_backend`/`rerank_model`) al dataclass `Settings` rompió 3 tests en CI (`test_sentence_transformer_embedder.py::TestBuildEmbedder`) cuyo helper `_settings()` construía `Settings(...)` directo sin los campos nuevos → `TypeError: missing required arguments`. El run local de esa PR solo corrió los tests nuevos (cross-encoder/hybrid), no el fichero afectado, así que CI lo pilló primero. Causa: (1) extender un dataclass compartido sin actualizar TODOS sus constructores directos; (2) verificar solo los tests nuevos en vez de la suite completa ante un cambio de esquema compartido. Fix: al añadir un campo a `Settings` (u otro dataclass compartido), `grep "Settings("` y actualizar cada constructor — o que los helpers de test deriven con `dataclasses.replace(get_settings(), ...)` para sobrevivir campos nuevos; y correr la suite COMPLETA en local antes de pushear un cambio de esquema, no solo los tests nuevos (#544).
- **2026-06-08** — `Path.write_text(json.dumps(meta, ensure_ascii=False))` en el caché del índice semántico petó con `UnicodeEncodeError` SOLO en Windows: `write_text` usa la codificación del locale (cp1252) por defecto, y el texto legal real de legalize-es trae caracteres fuera de cp1252 (catalán `ŀ`, el `…` de truncado). El 500 resultante volvió 422→500 un test de validación, que CI (Linux, utf-8 por defecto) nunca habría pillado; el run local sí. `graph/cache.py` se salva porque usa `ensure_ascii=True` (ASCII puro). Causa: confiar en la codificación por defecto de `write_text`/`read_text`. Fix: SIEMPRE pasar `encoding="utf-8"` explícito en I/O de ficheros de texto (read y write), sobre todo si el contenido es no-ASCII; correr la suite en local (Windows) antes de fiarse solo de CI Linux para bugs de encoding (#535).
- **2026-06-08** — El `stream_chat` de Ollama gateaba el parseo con `if isinstance(chunk, dict)`, pero el SDK real (ollama 0.6.1) emite `ChatResponse` (un `SubscriptableBaseModel`, NO `dict`) → en producción descartaba TODOS los tokens aunque los tests con fakes-dict pasaban. Causa: asumir la forma del chunk sin verificar el tipo real del SDK; los fakes-dict ocultaron el bug. Fix: acceder con `.get()` (funciona igual para los objetos reales del SDK y para dicts) y NUNCA gatear el parseo de un provider con `isinstance(_, dict)`. Regla: todo test que toque el parseo de un SDK debe alimentar al menos UN objeto real del SDK, no solo dicts (#531).
- **2026-06-06** — Hacer `git stash` mientras un `pytest` en background lee del working tree deja el resultado contaminado: el subproceso lee archivos en estado intermedio, los módulos ya importados retienen las clases viejas, los nuevos imports ven los archivos restaurados. Causa: el stash modifica el disco bajo los pies de un proceso en marcha. Fix: ANTES de stashear, o (a) `wait` a que el background process termine, o (b) kill explícito (`taskkill`/`pkill`) y re-run después de unstash. NUNCA confiar en un resultado de pytest si se hizo stash mientras corría.
- **2026-06-06** — `rtk gh issue create --body "$(cat <<'EOF' ... EOF)"` con cuerpos largos rompe el parser de bash con `unexpected EOF while looking for matching '`. Causa: ciertos caracteres dentro del heredoc (comillas combinadas con escapes de markdown, backticks anidados) confunden el lexer cuando se invoca a través del wrapper `rtk`. Fix: para issues/PRs con cuerpo largo, escribir el body a un fichero temporal y usar `gh issue create --body-file .tmp-body.md`, luego `rm` el fichero. Más robusto que el heredoc inline en cualquier shell.
- **2026-06-06** — Crear un worktree fresco vía `git worktree add` y correr `vitest run` falla con `ERR_MODULE_NOT_FOUND` porque el directorio nunca tuvo `npm install`. Causa: el worktree comparte `.git` pero NO `node_modules` (que es gitignored). Fix: tras `git worktree add ../<name>`, **siempre** `cd <name>/frontend && npm install --no-fund --no-audit` antes de correr vitest/build. `npx tsc` sí funciona (no necesita el bundle de runtime) pero los test runners y el build sí.
- **2026-06-06** — `dorny/paths-filter@v3` puesto como PRIMER step del job rompe el evento `push` con `git exit 128`. Causa: en PRs la action consulta la GH API y no necesita repo, pero en `push` cae a `git diff` y necesita el checkout con historia. Fix: `actions/checkout@v6` con `fetch-depth: 0` SIEMPRE antes del filter step en cualquier job gateado. La submódulo (legalize-es) sigue opt-out del checkout inicial (`submodules: false`) y se clona manualmente con shallow + blob:none después del filter. Aplica a cualquier patrón filter+gate en CI: el checkout va PRIMERO.
...
- **2026-05-22** — El patrón `lib/` heredado del template estándar de `.gitignore` de Python silenciosamente excluyó `frontend/src/lib/` entero al commitear el scaffold React (PR #70), dejando `dev` con un frontend que no compila. Causa: las reglas sin slash inicial son globales — matchean cualquier subdirectorio del mismo nombre, no solo el raíz. Fix: anclar al raíz con `/lib/` y `/lib64/`. Aplica a TODO patrón en `.gitignore` que se refiera a un directorio Python específico de raíz (`/build/`, `/dist/` ya estaban bien). Regla general: cuando un patrón tiene un nombre genérico, anclalo con `/` inicial salvo que de verdad lo quieras global. Verificar con `git check-ignore -v <path>` antes de commitear cualquier scaffold grande.
...
- **2026-05-30** — En #230 escribí código contra una API de grafo **asumida** (`GraphNode`/`GraphEdge`/`add_reference(edge)`) que no existía — la real es `add_law(metadata)` + `add_reference(source, target, *, source_article, reference_text)`. Varias llamadas `Edit` fallaron en silencio (string-not-found) pero igualmente commiteé y force-pusheé DOS veces un estado roto, con mensajes de commit que mentían ("266/271 passed") cuando la suite ni corría (ImportError). Causa: batchear muchas Read/Edit/Bash en paralelo sin confirmar las firmas reales, y fiarme de outputs replayeados en vez de verificar el archivo real + tests verdes. Fix: (1) ANTES de llamar a otro módulo, leer sus definiciones reales — nunca inventar nombres de clase/función de memoria; (2) si un `Edit` posterior sobre el mismo archivo falla con "string not found", PARAR y re-leer, el archivo no es lo que crees; (3) NUNCA escribir un recuento de tests ni "green" en un mensaje de commit hasta que `uv run pytest` haya pasado de verdad en esta sesión; correr format+lint+mypy+tests en secuencia (no en un batch paralelo gigante) y leer cada resultado; (4) no commitear/pushear hasta confirmar verde. **Cómo arreglarlo si ya pasó**: añadir los cambios que faltan, verificar verde de verdad, `git commit --amend`, y `git push --force-with-lease` (requiere OK explícito del usuario — el clasificador bloquea force-push). Si el `uv.lock` se cuela en el commit, `git checkout origin/main -- uv.lock` antes de amend. Memoria larga: `memory/feedback-read-before-coding-apis.md`.
