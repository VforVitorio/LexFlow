...
class OpenAIProvider(ChatProvider):
    ...
    async def stream_chat_typed(
        ...
        The wire adaptation lives in ``_openai_compat.stream_tool_use``
        (shared with LM Studio, which is OpenAI-compatible). This wrapper
        only adds OpenAI's SDK-error → ``ChatProviderError`` mapping.
        """
        try:
